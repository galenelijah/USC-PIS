...
const Sidebar = ({ mobileOpen, handleDrawerToggle }) => {
  ...
  const patientItems = [
    ...
    { text: 'Health Insights & History', icon: <InsightsIcon />, path: '/health-insights', description: 'Visual health timeline, trends, and chronological history of all medical events', requiredRole: ['ADMIN', 'STAFF', 'DOCTOR', 'DENTIST', 'NURSE', 'STUDENT', 'FACULTY'] },
    {
      ...
      icon: <AssignmentIcon />,
      path: '/health-records',
      description: ['STUDENT', 'FACULTY'].includes(user?.role)
    ...
    { text: 'Medical Certificates', icon: <CertificateIcon />, path: '/medical-certificates', requiredRole: ['ADMIN', 'STAFF', 'DOCTOR', 'DENTIST', 'NURSE', 'STUDENT', 'FACULTY'] },
  ];
  ...
  return (
    ...
    >
      {/* Mobile drawer - Always scrollable, customized scrollbar */}
      <Drawer
        variant="temporary"
        open={mobileOpen}
        ...
        ModalProps={{
          ...
          disableAutoFocus: true, // Prevents sudden auto-focus collisions
          disableEnforceFocus: true, // Softens strict modal focus locks
          disableScrollLock: true, // <-- ADD THIS LINE TO PREVENT ARIA-HIDDEN ACCESSIBILITY COLLISIONS
        }}
        SlideProps={{
          unmountOnExit: true, // Cleanly tears down state when closed
        }}
        sx={{
      ...
      {/* Desktop drawer - Scroll on hover only */}
      <Drawer
        variant="permanent"
        sx={{
          ...
          '& .MuiDrawer-paper': {
            ...
            backgroundColor: '#1a5e20',
            color: 'white',
            borderRadius: 0, // <-- Ensures the sidebar itself is perfectly straight
            borderRight: 'none', // Disables default grey border rules
