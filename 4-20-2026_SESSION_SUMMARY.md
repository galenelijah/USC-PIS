...
*   **Self-Service Role Selection:** Enabled a secure "Self-Update" mechanism for users with the `STUDENT` role. They can now choose their specific role (TEACHER, STAFF, DOCTOR, etc.) via the `/role-selection` screen. Security constraints prevent self-assignment of the `ADMIN` role and block further changes once a role is chosen.
...
*   **Teacher Implementation:** Revised `TEACHER_ROLE_IMPLEMENTATION_GUIDE.md` to align with the post-verification role selection architecture.
