# Faculty Role Implementation Guide

## Overview
This document details the implementation of the `FACULTY` role in the USC-PIS system. The goal was to allow faculty members to use the clinic's services as patients (similar to Students) while distinguishing them from administrative `STAFF`.

## Architecture

### 1. Role Definition
The `FACULTY` role is defined in `backend/authentication/models.py` within the `User.Role` choices.
*   **Database Value:** `FACULTY`
*   **Label:** "Faculty"
...
*   **Patient Roles:** `STUDENT`, `FACULTY`
*   **Staff Roles:** `ADMIN`, `DOCTOR`, `NURSE`, `STAFF`, `DENTIST`

### 3. Registration Logic
The challenge was differentiating Facultys from Staff, as both often use text-only emails (e.g., `name@usc.edu.ph`), whereas Students typically use number-based emails.
...
1.  **Backend (`serializers.py`):**
    *   `_determine_role_from_email` logic:
        ...
        *   If email has no digits -> Default to `STUDENT` (Allows self-selection of `FACULTY` only).
...
3.  **Role Selection (`RoleSelection.jsx`):**
    *   **Self-Service:** Users can self-identify as `FACULTY` (Faculty) or continue as `STUDENT`.
...
4.  **Backend Permissions (`user_management_views.py`):**
    *   The `update_user_role` endpoint enforces:
        *   **Self-Update:** Only allowed for `STUDENT` $\rightarrow$ `FACULTY`.
...
Facultys need a `Patient` profile in the database to store medical records.
*   **Backend:** `CompleteProfileSetupView` in `views.py` was updated to check `if user.role in [User.Role.STUDENT, User.Role.FACULTY]:`.
    *   **Field Forking:** For `FACULTY` roles, the setup requires `department` information instead of `course/year`.
*   **Frontend:** `ProfileSetup.jsx` treats `isStudent` as true if the role is `STUDENT` or `FACULTY`, but forks the UI labels to show "Department" for facultys.

### 5. Access Control & Management (Refined March 2026)
Global changes were made to ensure Facultys have the same view access as Students while maintaining administrative clarity.
...
*   **Patient List Filtering:** The `/patients` management list automatically filters for `STUDENT` and `FACULTY` roles. If a user's role is changed to an administrative one, they are automatically removed from the active patient list.
*   **Data Mapping:** The `PatientSerializer` correctly maps a Faculty's `department` to the "Course/Department" column in the patients list for consistent reporting.
...
*   If Facultys require specific data fields different from Students (e.g., Department instead of Course), the `ProfileSetup` form will need to fork logic based on the specific role rather than the shared "Patient Role".
