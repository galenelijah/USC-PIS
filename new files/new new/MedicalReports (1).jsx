...
const MedicalReports = ({ dateRange, customStart, customEnd }) => {
  ...
  const fetchProviderName = useCallback(async (id) => {
    if (providerMap[String(id)]) return; // Already fetched
    ...
    try {
      ...
      if (userData) {
        ...
        setProviderMap(prev => ({ ...prev, [String(id)]: fullName }));
      }
    } catch (err) {
      console.error(`Error fetching user ${id}:`, err);
      setProviderMap(prev => ({ ...prev, [String(id)]: `User #${id}` }));
    }
  }, [providerMap]);
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        ...
        // Resolve courses
        const cMap = {};
        ProgramsChoices.forEach(p => { cMap[String(p.id)] = p.label; });
        setCourseMap(cMap);

        // Batch trigger name fetching for unique providers
        const uniqueProviderIds = Array.from(new Set(dataList.map(r => r.created_by).filter(Boolean)));
        uniqueProviderIds.forEach(id => fetchProviderName(id));
